...
public class JoinCommand extends GunWarCommand implements LoggableDefault {

    public JoinCommand() {
        super("join", "", "Usage: /join", Arrays.asList("j"));
    }
    ...
    public boolean execute(CommandSender sender, String label, String[] args) {
        if(sender instanceof Player) {
            ...
            if(d.isJoined()) {
                sender.sendMessage(TextReference.CHAT_PREFIX + " " + TextReference.CHAT_GAME_ERROR_ALREADY_JOINED);
                return true;
            }
            d.join();
            Bukkit.broadcastMessage(TextReference.CHAT_PREFIX + " " + TextReference.CHAT_GAME_PLAYER_JOIN);
            return true;
        }
